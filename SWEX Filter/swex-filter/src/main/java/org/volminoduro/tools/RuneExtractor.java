...
public final class RuneExtractor {
    ...
    public static Collection<Rune> extractAllRunes() {
        ...
        for (JsonNode monster : monsterList) {
            ...
            monsterJSON.runesJSON().forEach(rune -> runes.add(RuneMapper.translateRuneJSON(rune, Builder.buildMonsterFromMonsterJSON(monsterJSON))));
        }
    ...
    private static Collection<Rune> extractRunesFromJsonNodeListOfRunes(JsonNode jsonNode) {
        Collection<Rune> runes = new ArrayList<>();
        jsonNode.forEach(runeJsonNode -> runes.add(RuneMapper.translateRuneJSON(Builder.buildRuneJSONFromJsonNode(runeJsonNode), null)));
        return runes;
    }
}
